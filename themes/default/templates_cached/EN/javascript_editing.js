...
{var cookie=read_cookie('use_wysiwyg');return((cookie=='')||(cookie=='1'))&&(browser_matches('wysiwyg')&&('0'!='1'));}
function wysiwyg_on()
...
{window.fauxmodal_confirm('Would you like to keep WYSIWYG disabled? (this will be recorded on your computer using a cookie)',function(saving_cookies)
{_toggle_wysiwyg(name,saving_cookies);},'Remember setting?');return false;}
return _toggle_wysiwyg(name,true);}
function _toggle_wysiwyg(name,saving_cookies)
{var is_wysiwyg_on=wysiwyg_on();if(saving_cookies)
{set_cookie('use_wysiwyg',is_wysiwyg_on?'0':'1',3000);}
var forms=document.getElementsByTagName('form');var so=document.getElementById('post_special_options');var so2=document.getElementById('post_special_options2');if(is_wysiwyg_on)
...
{for(var counter=0;counter<forms[fid].elements.length;counter++)
{var id=forms[fid].elements[counter].id;if(typeof window.wysiwyg_editors[id]!='undefined')
{var textarea=forms[fid].elements[counter];document.getElementById(id+'__is_wysiwyg').value='0';textarea.style.display='block';textarea.style.visibility='visible';textarea.disabled=false;textarea.readOnly=false;var wysiwyg_data=window.wysiwyg_editors[id].getData();window.wysiwyg_editors[id].destroy();delete window.wysiwyg_editors[id];if((discard)&&(window.wysiwyg_original_comcode[id]))
{textarea.value=window.wysiwyg_original_comcode[id];}else
{var url=maintain_theme_in_link('/combs/data/comcode_convert.php?from_html=1'+keep_stub());if(window.location.href.indexOf('topics')!=-1)url+='&forum_db=1';var request=do_ajax_request(url,false,'data='+window.encodeURIComponent(wysiwyg_data.replace(new RegExp(String.fromCharCode(8203),'g'),'')));if((!request.responseXML)||(!request.responseXML.documentElement.getElementsByTagName('result')[0]))
...
set_inner_html(document.getElementById('toggle_wysiwyg_'+id),'Enable WYSIWYG editing');}}}
if(so)so.style.display='block';if(so2)so2.style.display='none';window.wysiwyg_on=function(){return false;};}
window.wysiwyg_editors={};window.wysiwyg_original_comcode={};function load_html_edit(posting_form,ajax_copy)
...
{e=posting_form.elements[counter];id=e.id;if((e.type=='textarea')&&(e.className.indexOf('wysiwyg')!=-1)&&(!is_comcode_xml(e)))
...
set_inner_html(document.getElementById('toggle_wysiwyg_'+id),'Disable WYSIWYG editing');window.wysiwyg_original_comcode[id]=e.value;if(!ajax_copy)
...
{var url=maintain_theme_in_link('/combs/data/comcode_convert.php?semihtml=1&from_html=0'+keep_stub());if(window.location.href.indexOf('topics')!=-1)url+='&forum_db=1';var request=do_ajax_request(url,false,'data='+window.encodeURIComponent(posting_form.elements[counter].value.replace(new RegExp(String.fromCharCode(8203),'g'),'').replace('{'+'$,page hint: no_wysiwyg}','')));if(!request.responseXML)
...
window.setTimeout(function(e,id){return function(){window.wysiwyg_editors[id]=wysiwyg_editor_init_for(e);}}(e,id),1000);}}
if(count==0)return;}
...
var precision_editing=((typeof window.take_errors!='undefined')&&window.take_errors)||(get_elements_by_class_name(document.body,'comcode_button_box').length>1);var toolbar=[];if(precision_editing)
toolbar.push(['Source','-']);toolbar.push(['Cut','Copy','Paste',precision_editing?'PasteText':null,precision_editing?'PasteFromWord':null]);toolbar.push(['Undo','Redo',precision_editing?'-':null,precision_editing?'Find':null,precision_editing?'Replace':null,'-',precision_editing?'SelectAll':null,'RemoveFormat']);toolbar.push(['Link','Unlink']);toolbar.push(precision_editing?'/':'-');var formatting=['Bold','Italic','Strike','-',precision_editing?'Subscript':null,precision_editing?'Superscript':null];toolbar.push(formatting);toolbar.push(['NumberedList','BulletedList',precision_editing?'-':null,precision_editing?'Outdent':null,precision_editing?'Indent':null]);if(precision_editing)
toolbar.push(['JustifyLeft','JustifyCenter','JustifyRight',precision_editing?'JustifyBlock':null]);toolbar.push([precision_editing?'ocportal_image':null,'Table']);if(precision_editing)
...
toolbar.push(['HorizontalRule','SpecialChar']);var use_ocportal_toolbar=true;if(use_ocportal_toolbar)
toolbar.push(['ocportal_block','ocportal_comcode',precision_editing?'ocportal_page':null,'ocportal_quote',precision_editing?'ocportal_box':null,'ocportal_code']);var editor_settings={enterMode:window.CKEDITOR.ENTER_BR,shiftEnterMode:window.CKEDITOR.ENTER_P,uiColor:wysiwyg_color,fontSize_sizes:'0.6em;0.85em;1em;1.1em;1.2em;1.3em;1.4em;1.5em;1.6em;1.7em;1.8em;2em',removePlugins:'',extraPlugins:''+(use_ocportal_toolbar?'ocportal':''),removeButtons:'PasteFromWord',customConfig:'',bodyId:'htmlarea',baseHref:get_base_url()+'/',linkShowAdvancedTab:true,imageShowAdvancedTab:true,imageShowLinkTab:true,imageShowSizing:true,autoUpdateElement:true,contentsCss:page_stylesheets,cssStatic:css,startupOutlineBlocks:true,language:(window.ocp_lang)?ocp_lang.toLowerCase():'en',emailProtection:false,resize_enabled:true,width:(find_width(element)-15),height:(window.location.href.indexOf('cms_comcode_pages')==-1)?250:500,disableNativeSpellChecker:false,toolbar:toolbar,allowedContent:true};if(typeof window.CKEDITOR.instances[element.id]!='undefined'&&window.CKEDITOR.instances[element.id])delete window.CKEDITOR.instances[element.id];var editor=window.CKEDITOR.replace(element.id,editor_settings);if(!editor)return;linked_sheets=document.getElementsByTagName('style');var css='';css+='body { width: 100%; min-height: 140px; }';css+="#screen_title { display: block !important }";css+=".MsoNormal { margin: 0; }";css+='kbd.ocp_keep,kbd.ocp_keep_block { background-color: #BABAFF; }';css+='input.ocp_keep_ui_controlled,input.ocp_keep_ui_controlled:focus { border:1px dotted; text-align: center; color: buttontext; background: buttonface; max-width: 270px; }';css+='input.ocp_keep_ui_controlled::selection { background: buttonface; }';css+='input.ocp_keep_ui_controlled::-moz-selection { background: buttonface; }';css+='.comcode_fake_table > div, .fp_col_block { outline: 1px dotted; margin: 1px 0; }';for(counter=0;counter<linked_sheets.length;counter++)
...
window.lang_PREFER_OCP_ATTACHMENTS='You will be taken to add a raw image. Note that the website has an attachments feature (underneath the editor, the Attachments section). Attachments come with an easy upload and sophisticated document display settings.';editor.on('key',function(event){if(typeof element.externalonKeyPress!='undefined')
{element.value=editor.getData();element.externalonKeyPress(event,element);}});editor.on('instanceReady',function(event){find_tags_in_editor(editor,element);});window.setInterval(function(){if(is_wysiwyg_field(element))
find_tags_in_editor(editor,element);},1000);editor.on('paste',function(event){if(event.data.html)
{event.data.html=event.data.html.replace(/<meta charset="utf-8">/g,'');event.data.html=event.data.html.replace(/<br class="Apple-interchange-newline">/g,'<br>');event.data.html=event.data.html.replace(/<div style="text-align: center;"><font class="Apple-style-span" face="'Lucida Grande'"><span class="Apple-style-span" style="font-size: 11px; white-space: pre;"><br><\/span><\/font><\/div>$/,'<br><br>');}});return editor;}
function find_tags_in_editor(editor,element)
{if(!editor.document)return;if(typeof editor.document.$=='undefined')return;if(!editor.document.$)return;var comcodes=get_elements_by_class_name(editor.document.$.getElementsByTagName('body')[0],'(ocp_keep|ocp_keep_block|ocp_keep_ui_controlled)');for(var i=0;i<comcodes.length;i++)
{if(!comcodes[i].onmouseout)
{comcodes[i].orig_title=comcodes[i].title;comcodes[i].onmouseout=function(event){if(typeof event=='undefined')var event=editor.window.$.event;var eventCopy={};if(event)
{if(event.pageX)eventCopy.pageX=3000;if(event.clientX)eventCopy.clientX=3000;if(event.pageY)eventCopy.pageY=3000;if(event.clientY)eventCopy.clientY=3000;if(typeof window.deactivate_tooltip!='undefined')deactivate_tooltip(this,eventCopy);}};comcodes[i].onmousemove=function(event){if(typeof event=='undefined')var event=editor.window.$.event;var eventCopy={};if(event)
...
{reposition_tooltip(this,eventCopy);this.title=this.orig_title;}}};comcodes[i].onmousedown=function(event){if(typeof event=='undefined')var event=editor.window.$.event;if(event.altKey)
{var range=document.selection.getRanges()[0];range.startOffset=0;range.endOffset=0;range.select()
document.selection.selectRanges([range]);}}
if(comcodes[i].nodeName.toLowerCase()=='input')
{comcodes[i].readOnly=true;comcodes[i].contentEditable=true;comcodes[i].ondblclick=function(event){if(this.onmouseout)this.onmouseout();var field_name=editor.name;if((typeof window.event!='undefined')&&(window.event))window.event.returnValue=false;if(this.id=='')this.id='comcode_tag_'+Math.round(Math.random()*10000000);var tag_type=this.title.replace(/^\[/,'').replace(/[= \]].*$/,'');if(tag_type=='block')
{var block_name=this.title.replace(/\[\/block\]$/,'').replace(/^(.|\s)*\]/,'');var url='http://localhost/combs/data/block_helper.php?type=step2&block='+window.encodeURIComponent(block_name)+'&field_name='+field_name+'&parse_defaults='+window.encodeURIComponent(this.title)+'&save_to_id='+window.encodeURIComponent(this.id)+keep_stub();url=url+'&block_type='+(((field_name.indexOf('edit_panel_')==-1)&&(window.location.href.indexOf(':panel_')==-1))?'main':'side');window.faux_open(maintain_theme_in_link(url),'','width=750,height=520,status=no,resizable=yes,scrollbars=yes',null,'Cancel');}else
{var url='http://localhost/combs/data/comcode_helper.php?type=step2&tag='+window.encodeURIComponent(tag_type)+'&field_name='+field_name+'&parse_defaults='+window.encodeURIComponent(this.title)+'&save_to_id='+window.encodeURIComponent(this.id)+keep_stub();window.faux_open(maintain_theme_in_link(url),'','width=750,height=520,status=no,resizable=yes,scrollbars=yes',null,'Cancel');}
return false;}}
comcodes[i].onmouseover=function(event){if(typeof event=='undefined')var event=editor.window.$.event;cancel_bubbling(event);if(typeof window.activate_tooltip!='undefined')
...
{self_ob.tag_text=tag_text;self_ob.is_over=true;var url=maintain_theme_in_link('/combs/data/comcode_convert.php?css=1&javascript=1&box_title=Preview'+keep_stub());if(window.location.href.indexOf('topics')!=-1)url+='&forum_db=1';var request=do_ajax_request(url,function(ajax_result_frame,ajax_result){if(ajax_result)
{var tmp_rendered=merge_text_nodes(ajax_result.childNodes);if(tmp_rendered.indexOf('Comcode validation error')==-1)
...
{if(typeof window.do_ajax_request=='undefined')return false;if(typeof window.merge_text_nodes=='undefined')return false;var element=document.getElementById(name);if(is_wysiwyg_field(element))
{window.fauxmodal_alert('You cannot convert to Comcode XML whilst the WYSIWYG editor is enabled.');return false;}
var old_text=element.value;var url=maintain_theme_in_link('/combs/data/comcode_convert.php?to_comcode_xml=1'+keep_stub());if(window.location.href.indexOf('topics')!=-1)url+='&forum_db=1';var request=do_ajax_request(url,false,'data='+window.encodeURIComponent(old_text).replace(new RegExp(String.fromCharCode(8203),'g'),''));var result=((request)&&(request.responseXML)&&(request.responseXML.documentElement))?request.responseXML.documentElement.getElementsByTagName('result')[0]:null;if((result)&&(result.childNodes[0].data))element.value=merge_text_nodes(result.childNodes);else
{var error_window=window.open();error_window.document.write(request.responseText);error_window.document.close();window.fauxmodal_alert('Your Comcode could not be parsed and hence could not be converted. You will be shown a Comcode debugger if you try to submit this form. Alternatively, there could have been a connection error.');}
return false;}
function do_emoticon(field_name,p,_opener)
{var element;if(_opener)
{element=get_main_ocp_window().document.getElementById(field_name);}else
{element=document.getElementById(field_name);}
element=ensure_true_id(element,field_name);var title=p.title;if(title=='')title=p.getElementsByTagName('img')[0].alt;title=title.replace(/^.*: /,'');var text=is_comcode_xml(element)?('<emoticon>'+escape_html(title)+'</emoticon>'):(' '+title+' ');if(_opener)
...
{if(!get_main_ocp_window().wysiwyg_editors)return;if(typeof description=='undefined')var description='';var element=get_main_ocp_window().document.getElementById(field_name);element=ensure_true_id(element,field_name);var comcode;if(!is_comcode_xml(element))
{comcode='\n\n[attachment type="island" description="'+escape_comcode(description)+'"]'+id+'[/attachment]';}else
...
{if(is_wysiwyg_field(element))
{if(typeof html=='undefined')var html=escape_html(text).replace(new RegExp('\\\\n','gi'),'<br />');window.wysiwyg_editors[element.id].setData(html);window.setTimeout(function(){find_tags_in_editor(window.wysiwyg_editors[element.id],element);},100);}
element.value=text;}
...
{var is_block=text.match(/^\s*\[block(.*)\](.*)\[\/block\]\s*$/);var is_non_text_tag=false;var non_text_tags=['section_controller','big_tab_controller','img','currency','contents','concepts','attachment','attachment_safe','flash','menu','email','reference','upload','page','exp_thumb','exp_ref','thumb','snapback','post','thread','topic','include','random','jumping','shocker'];for(var i=0;i<non_text_tags.length;i++)
is_non_text_tag=is_non_text_tag||text.match(new RegExp('^\s*\\['+non_text_tags[i]+'([ =].*)?\\](.*)\\[\/'+non_text_tags[i]+'\\]\s*$'));if(is_block||is_non_text_tag)
{var button_text=is_block?'{1} (dbl-click to edit/delete)':'{1} tag (dbl-click to edit/delete)';var matches=text.match(/^\s*\[(\w+)([ =].*)?\](.*)\[\/\w+\]\s*$/);insert=get_selected_html(editor)+
('<input class="ocp_keep_ui_controlled" size="45" title="'+(html?matches[0].replace(/^\s*/,'').replace(/\s*$/,''):escape_html(matches[0].replace(/^\s*/,'').replace(/\s*$/,'')))+'" readonly="readonly" type="text" value="'+(button_text.replace(/^(main|side|bottom)\_/,'').replace('{1}',matches[is_block?3:1]))+'" />');}else
{var tag_name=text.replace(/^\[/,'').replace(/[ \]].*$/,'');insert=get_selected_html(editor)+
...
{if(!browser_matches('opera'))editor.focus();var selected_html=get_selected_html(editor);if(browser_matches('opera'))editor.getSelection().getNative().getRangeAt(0).deleteContents();if((editor.getSelection())&&(editor.getSelection().getStartElement().getName()=='kbd'))
...
return;}
var from=element.value.length,to;element.focus();if(typeof sel=='undefined')var sel=null;if(sel===null)sel=document.selection?document.selection:null;if(typeof element.selectionEnd!='undefined')
...
{if((typeof sel=='undefined')||(!sel))var sel=get_main_ocp_window().document.selection?get_main_ocp_window().document.selection:null;get_main_ocp_window().insert_textbox(element,text,sel,plain_insert,html);}
...
{after_wrap_tag='[/'+before_wrap_tag+']';before_wrap_tag='['+before_wrap_tag+']';}else
{after_wrap_tag='</'+before_wrap_tag+'>';before_wrap_tag='<'+before_wrap_tag+'>';}}
if(is_wysiwyg_field(element))
{var editor=window.wysiwyg_editors[element.id];if(!browser_matches('opera'))editor.focus();var selected_html=get_selected_html(editor);if(browser_matches('opera'))editor.getSelection().getNative().getRangeAt(0).deleteContents();if(selected_html=='')selected_html='Text or Comcode goes here'.toUpperCase();var new_html='&#8203;<kbd title="'+escape_html(before_wrap_tag.replace(/^\[/,'').replace(/[ \]].*$/,'').replace(/=.*$/,''))+'" class="ocp_keep">'+before_wrap_tag+selected_html+after_wrap_tag+'</kbd>&#8203;';if((editor.getSelection())&&(editor.getSelection().getStartElement().getName()=='kbd'))
{editor.document.getBody().appendHtml(new_html);}else
{editor.insertHtml(new_html);}
find_tags_in_editor(editor,element);return;}
...
{input.focus();input.set_selection_range(selectionStart,selectionEnd);}
else if(typeof input.createTextRange!='undefined')
{var range=input.createTextRange();range.collapse(true);range.moveEnd('character',selectionEnd);range.moveStart('character',selectionStart);range.select();}else input.focus();}