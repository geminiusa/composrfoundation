...
"use strict";new Image().src='http://localhost/combs/themes/default/images/loading.gif'.replace(/^https?:/,window.location.protocol);function password_strength(ob)
{if(ob.name.indexOf('2')!=-1)return;if(ob.name.indexOf('confirm')!=-1)return;var _ind=document.getElementById('password_strength_'+ob.id);if(!_ind)return;var ind=_ind.getElementsByTagName('div')[0];var strength=load_snippet('password_strength','password='+window.encodeURIComponent(ob.value));strength*=2;if(strength>10)strength=10;ind.style.width=(strength*10)+'px';if(strength>=6)
...
{var error_element=null;if(typeof the_element.name!='undefined')
{var id=the_element.name;error_element=document.getElementById('error_'+id);if(!error_element)
{if((error_msg=='')&&(id.indexOf('_hour')!=-1)||(id.indexOf('_minute')!=-1))return;error_element=document.getElementById('error_'+id.replace(/\_day$/,'').replace(/\_month$/,'').replace(/\_year$/,'').replace(/\_hour$/,'').replace(/\_minute$/,''));}
if(error_element)
{error_element.style.display=(error_msg=='')?'none':'block';if(get_inner_html(error_element)!=escape_html(error_msg))
{set_inner_html(error_element,'');if(error_msg!='')
{the_element.setAttribute('aria-invalid','true');var p=error_element;while(p!==null)
...
{select_tab('g',p.getAttribute('id').substr(2,p.id.length-2),true);break;}}
var msg_node=document.createTextNode(error_msg);error_element.appendChild(msg_node);error_element.setAttribute('role','alert');if(typeof window.fade_transition!='undefined')
{set_opacity(error_element,0.0);fade_transition(error_element,100,30,4);}}else
{the_element.setAttribute('aria-invalid','false');error_element.setAttribute('role','');}}}}
...
document.getElementById('submit_button').style.display='inline';var pf=document.getElementById('preview_iframe');window.setInterval(window.trigger_resize,500);animate_frame_load(pf,'preview_iframe',50);return true;}
function clever_find_value(the_form,the_element)
{var my_value=(typeof window.get_textbox=='undefined')?the_element.value:get_textbox(the_element);if(the_element.getAttribute('type')=='radio')
{my_value='';for(var i=0;i<the_form.elements.length;i++)
...
{my_value=the_element.options[the_element.selectedIndex].value;if((my_value=='')&&(the_element.getAttribute('size')>1))my_value='-1';}
if(my_value===null)my_value='';return my_value;}
function check_field(the_element,the_form,for_preview)
{var i,the_class,required,my_value,erroneous=false,error_msg='',regexp,total_file_size=0,alerted=false,error_element=null;if(((the_element.type=='hidden')||((the_element.style.display=='none')&&((typeof window.is_wysiwyg_field=='undefined')||(!is_wysiwyg_field(the_element)))))&&((!the_element.className)||(element_has_class(the_element,'hidden_but_needed'))==null))
{return null;}
...
total_file_size+=the_element.files.item(0).fileSize;if((the_element.type=='file')&&(the_element.value)&&(the_element.name!='file_novalidate'))
{var valid_types='3gp,asf,avi,bmp,css,csv,diff,dmg,doc,docx,dot,eml,f4v,flv,gif,gz,ico,ini,iso,jpeg,jpg,log,m4v,mov,mp3,mp4,mpeg,mpg,odg,odp,ods,odt,ogg,ogv,patch,pdf,pgp,php,png,ppt,pptx,psd,pub,rar,sql,swf,tar,tga,tif,torrent,tpl,txt,wav,webm,wmv,xls,xlsx,zip'.split(/,/);var type_ok=false;var theFileType=the_element.value.indexOf('.')?the_element.value.substr(the_element.value.lastIndexOf('.')+1):'None';for(var k=0;k<valid_types.length;k++)
...
the_class=first_class_name(the_element.className);if((!for_preview)&&(the_element.name=='delete')&&(((the_class=='input_radio')&&(the_element.value!='0'))||(the_class=='input_tick'))&&(the_element.checked))
{return[false,the_element,0,true];}
...
my_value=clever_find_value(the_form,the_element);if((required)&&((my_value.replace(/&nbsp;/g,' ').replace(/<br\s*\/?>/g,' ').replace(/\s/g,'')=='')||(my_value=='****')))
{error_msg='This is a required field so needs filling in';}else
...
{error_msg='You specified a non-valid e-mail address (e-mail addresses contain "@")'.replace('xxx',my_value);}
if(((the_class=='input_username')||(the_class=='input_username_required'))&&(my_value!='')&&(my_value!='****')&&(window.do_ajax_field_test)&&(!do_ajax_field_test('/combs/data/username_exists.php?username='+encodeURIComponent(my_value))))
{error_msg='xxx is not an active username'.replace('xxx',my_value);}
if(((the_class=='input_codename')||(the_class=='input_codename_required'))&&(my_value!='')&&(my_value!='****')&&(!my_value.match(/^[a-zA-Z0-9\-\.\_]*$/)))
{error_msg='A field that was supposed to be a codename (numbers and letters, no symbols) was not'.replace('xxx',my_value);}
if(((the_class=='input_integer')||(the_class=='input_integer_required'))&&(my_value!='')&&(my_value!='****')&&(parseInt(my_value,10)!=my_value-0))
{error_msg='A field that was supposed to be an integer (for our purposes, a whole number between -2147483648 and 2147483647) was not'.replace('xxx',my_value);}
if(((the_class=='input_float')||(the_class=='input_float_required'))&&(my_value!='')&&(my_value!='****')&&(parseFloat(my_value)!=my_value-0))
{error_msg='A field that was supposed to be a float (decimal number) was not'.replace('xxx',my_value);}}
set_field_error(the_element,error_msg);if((error_msg!='')&&(!erroneous))
{erroneous=true;error_element=the_element;}
...
{erroneous=check_result[0]||erroneous;if(!error_element)error_element=check_result[1];total_file_size+=check_result[2];alerted=check_result[3]||alerted;if(check_result[0])
...
{e=es[i];if((e.name.match(/^access_\d+_sp_/))&&((is_delete)||(e.options[e.selectedIndex].value=='-1')))
{e.disabled=true;}}}
return!erroneous;}
...
{if(typeof second_run=='undefined')var second_run=false;var fields=[];for(var i=0;i<field_names.length;i++)
...
for(var i=0;i<field_names.length;i++)
{var field=fields[i];if((field)&&(_standard_alternate_field_is_filled_in(field,second_run,false)))
...
{var is_set=force||((field.value!='')&&(field.value!='-1'))||((typeof field.virtual_value!='undefined')&&(field.virtual_value!='')&&(field.virtual_value!='-1'));var radio_button=document.getElementById('choose_'+field.name.replace(/\[\]$/,''));if(!radio_button)radio_button=document.getElementById('choose_'+field.name.replace(/\_\d+$/,'_'));if(second_run)
...
{var radio_button=document.getElementById('choose_'+field.name.replace(/\[\]$/,''));if(!radio_button)radio_button=document.getElementById('choose_'+field.name.replace(/\_\d+$/,'_'));if(radio_button)
...
{var field=document.getElementById(field_name);if(field)return field;var radio_buttons=[],i,j,e;radio_buttons['name']=field_name;radio_buttons['value']='';for(i=0;i<document.forms.length;i++)
...
{var radio_button=document.getElementById('choose_'+field.name.replace(/\[\]$/,''));if(!radio_button)radio_button=document.getElementById('choose_'+field.name.replace(/\_\d+$/,'_'));set_locked(field,is_locked,chosen_field);if(something_required)
...
{element.className=element.className.replace(/(input\_[a-z\_]+)_required/g,'$1');if(typeof element.swfob!='undefined')
...
{pic.src=((pic.src.indexOf('themewizard.php')!=-1)?pic.src.replace('expand','contract'):'http://localhost/combs/themes/default/images/contract.png').replace(/^https?:/,window.location.protocol);pic.setAttribute('alt','Contract');pic.setAttribute('title','Contract');new_state=(field_input.nodeName.toLowerCase()=='tr')?'table-row':'block';new_state_2='block';new_state_3='1px dashed';}else
{pic.src=((pic.src.indexOf('themewizard.php')!=-1)?pic.src.replace('contract','expand'):'http://localhost/combs/themes/default/images/expand.png').replace(/^https?:/,window.location.protocol);pic.setAttribute('alt','Expand');pic.setAttribute('title','Expand');new_state='none';new_state_2='none';new_state_3='0';}
...
if(typeof help_id=='undefined')var help_id=pic.parentNode.id+'_help';var help=document.getElementById(help_id);while(help!==null)
...
{if(e[i].disabled)continue;var img=e[i].parentNode.parentNode.parentNode.getElementsByTagName('img')[0];if((img)&&(img!=ob))
...
if(r.disabled)return;r.checked=true;if(typeof r.fakeonchange!='undefined'&&r.fakeonchange)r.fakeonchange(event);ob.parentNode.className+=' selected';if(!browser_matches('opera'))ob.style.outline='1px dotted';}
function disable_preview_scripts(under)
{if(typeof under=='undefined')var under=document;var elements,i;var no_go=function(){window.fauxmodal_alert('You are previewing, so button clicks have been disabled.');return false;};elements=under.getElementsByTagName('button');for(i=0;i<elements.length;i++)
...
{try
{if((!elements[i].href)||(elements[i].href.toLowerCase().indexOf('javascript:')!=0))
...
{if(this.checked)
{window.fauxmodal_confirm('Are you sure you want to do this?',function(result)
